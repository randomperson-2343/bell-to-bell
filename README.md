...
- **The household is cosmetic.** A shelter visit on session 11 offers a cat or a dog (Rosco if he is a boy, Jemma if she is a girl) for $150 and $35 a week in food. The pet lives in every weekend apartment scene, looked after or visibly neglected. A skipped dentist cleaning (session 16) can become a $1,400 toothache (session 39), a skipped vet checkup (session 26) a $900 emergency (session 41), and your father's surgery bill now comes on session 32. These only move your own money and change the art and the ending's closing lines: never the story, stress or which ending you reach, and a test holds them to that.
...
- **High-resolution pixel art**, rendered as cached authored storyboard frames: a locked 32-colour palette, a native 640x360 cinematic canvas scaled to fill the screen, 16:9 letterboxing, ordered dithering and bitmap-aligned typography. Every face is a pixel font: Silkscreen handles display and control lettering, Tiny5 carries interface and story copy, and VT323 is reserved for financial data, with font-size-adjust keeping their cap heights consistent. All fonts ship in `fonts/` (SIL Open Font License), so the game needs no network.
