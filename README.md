...
- **High-resolution pixel art**, rendered as cached authored storyboard frames: a locked 32-colour palette, a native 640x360 cinematic canvas, integer desktop scaling, 16:9 letterboxing, ordered dithering and bitmap-aligned typography. Every face is a pixel font: Silkscreen handles display and control lettering, Tiny5 carries interface and story copy, and VT323 is reserved for financial data, with font-size-adjust keeping their cap heights consistent. All fonts ship in `fonts/` (SIL Open Font License), so the game needs no network.
